...
export default class Payment extends React.Component {
  ...
  constructor(props){
    ...
    //COD
    if(config.enableCOD){
      paymentOptions.push({ id: 'cod', title: route.params.deliveryMethod=="delivery"?Language.cod:Language.cop },)
    }
    //Stripe
    ...
    if(config.enablePayPal){
      paymentOptions.push( { id: 'paypal', title: Language.pay_with_paypal });
    }

    //Mollie
    if(config.enableMollie){
      paymentOptions.push( { id: 'mollie', title: Language.pay_with_mollie });
    }

    //PayStack
    if(config.enablePayStack){
      paymentOptions.push( { id: 'paystack', title: Language.pay_with_paystack });
    }
    this.state = {
        order:[],
        totalQty:0,

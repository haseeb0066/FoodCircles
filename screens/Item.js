...
class Item extends React.Component {
  ...
  getSinglePrice(){
    var itemPrice=this.state.variantFound?this.state.variantPrice:this.state.item.price;
            ...
            >
              <Block flex style={styles.itemCard}>
                <Block middle style={styles.avatarContainer}>
                  <Image
                    source={{ uri: this.state.item.icon.indexOf('http')!=-1?this.state.item.icon:(config.domain.replace("api/v2","")+this.state.item.icon)     }}
                    style={styles.avatar}
                  />
                </Block>
